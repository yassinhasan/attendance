...
// $data['load_data']  =  toLink("admin/pharmacistsgroups/realtime");
...
//$data['edit_data']  =  toLink("admin/pharmacistsgroups/edit/");
        ...
        .then(data=>
            {
                if(data.error)
                {
                    if(data.error.users_id)
                    {
                        document.querySelector(".users-id").classList.add("alert" , "alert-danger");
                        document.querySelector(".users-id").innerHTML = data.error.users_id;
                    }
                    ...
                    if(data.error.pharmacy_id)
                    {
                        document.querySelector(".pharmacy-id").classList.add("alert" , "alert-danger");
                        document.querySelector(".pharmacy-id").innerHTML = data.error.pharmacy_id;
                    }
                    if(data.error.image)
                    {
                        document.querySelector(".image").classList.add("alert" , "alert-danger");
                        document.querySelector(".image").innerHTML = data.error.image;
                    }
                    removespinner()
                }
                if(data.suc)
                ...
                if(data.db_error)
                {
                    if(data.db_error)
                    {
                        document.querySelector(".pharmacists-id").classList.add("alert" , "alert-danger");
                        document.querySelector(".pharmacists-id").innerHTML = data.db_error;
                    }
                    removespinner()
                }
            }) 
...
function makeTable(results)
{
    ...
    if( Array.isArray(results) )
    {
        for (let index in  results) 
        {
           ...
           addedd_column += `<tr>
                            <td>
                            ${results[index].users_id}
                            </td>
                            <td class="table-img-div">
                            <img src="/Public/uploades/images/${results[index].image}" class="table-img">
                            </td>
                            <td>
                                ${results[index].firstname} ${results[index].lastname}
                            </td>
                            <td>
                                PH-${results[index].pharmacy_id}
                            </td>
                            <td>
...
//     == http://www.attendance.com/admin/pharmacistsgroups/edit/1
let edit_url; 
function edit(el)
{
    el.forEach(element => {
